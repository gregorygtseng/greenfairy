chrome.tabs.onActivated.addListener(function(activeInfo) { 
    ...
    //alert("This is the url of the tab = " + changeInfo.url);
  	climateCounts.getWindowLocation(tab); 
});
});
...
var climateCounts = {
    ...
    removeTlds: function(components) {
        ...
        for (var x in components) {
            for (var y in climateCounts.tldList) {
                if (components[x] == climateCounts.tldList[y] ) {
                    removeThese.unshift(x);
                }
            }
        }
        for (var z in removeThese) {
            components.splice(removeThese[z], 1)
        }
        return components[components.length - 1]
    },
    getWindowLocation: function (tab) {
        var self = this;
        var url = tab.url;
